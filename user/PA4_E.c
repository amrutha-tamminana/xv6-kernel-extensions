...
test_pa4_14(void)
{
    ...
    printf("  disk_reads=%d disk_writes=%d avg_disk_latency=%d.%d ticks\n",
           st.disk_reads, st.disk_writes, st.avg_disk_latency/100, st.avg_disk_latency%100);

    if (st.disk_writes > 0)
        printf("  PASS: swap-out (disk_writes) occurred\n");
    else
        printf("  FAIL: no disk_writes recorded — check swap/RAID path\n");

    if (st.disk_reads > 0)
        printf("  PASS: swap-in (disk_reads) occurred\n");
    else
        printf("  FAIL: no disk_reads recorded\n");
    ...
    int expected_min_disk_writes = SWAP_PAGES_14 * 4; // 4 blocks per page
    if (st.disk_writes >= expected_min_disk_writes / 4) // some fraction acceptable
        printf("  PASS: write count plausible for RAID0 striping\n");
    else
        printf("  NOTE: write count %d may be lower than expected %d\n",
               st.disk_writes, expected_min_disk_writes / 4);
...
test_pa4_15(void)
{
    ...
    // RAID1 disk_writes each block twice (primary + mirror)
    struct vmstats st;
    memset(&st, 0, sizeof(st));
    getvmstats(getpid2(), &st);
    printf("  disk_reads=%d disk_writes=%d avg_disk_latency=%d.%d\n",
           st.disk_reads, st.disk_writes, st.avg_disk_latency/100, st.avg_disk_latency%100);
    if (st.disk_writes > 0 && st.disk_reads > 0)
        printf("  PASS: I/O recorded\n");
    else
        printf("  FAIL: missing disk_reads or disk_writes\n");
...
test_pa4_16(void)
{
    ...
    // RAID5 disk_writes data + parity; expect more disk_writes than RAID0
    printf("  NOTE: RAID5 generates extra disk_writes for parity blocks\n");

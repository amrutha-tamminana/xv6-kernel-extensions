...
test_pa4_2(void)
{
    ...
    if (st.page_faults > 0 && st.page_faults <= NUM_PAGES_2*2)
        printf("PASS: page_faults (%d) in expected range [1, %d]\n",
               st.page_faults, NUM_PAGES_2*2);
    else
        printf("FAIL: expected page_faults in range [1, %d], got %d\n",
               NUM_PAGES_2*2, st.page_faults);
...
#define FRAME_LIMIT_7 128      // match kernel MAX_FRAMES

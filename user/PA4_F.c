...
test_pa4_18(void)
{
    ...
    int delta_disk_reads  = parent_after.disk_reads  - parent_before.disk_reads;
    int delta_disk_writes = parent_after.disk_writes - parent_before.disk_writes;
    printf("  parent delta: disk_reads=%d disk_writes=%d\n", delta_disk_reads, delta_disk_writes);
    if (delta_disk_reads == 0 && delta_disk_writes == 0)
    ...
    printf("  HI-priority: disk_reads=%d disk_writes=%d latency=%d.%d\n",
    ...
    printf("  LO-priority: disk_reads=%d disk_writes=%d latency=%d.%d\n",
    ...
    // Switch policy between disk_writes and disk_reads — data must still be correct
    ...
    setdisksched(SSTF_18); // switch policy before disk_reads
    int errs = 0;
    for (int i = 0; i < SWAP_PAGES_18; i++)
...
#define ROTATIONAL_C  5       // from param.h
#define FCFS_19          0
#define SSTF_19          1
...
test_pa4_19(void)
{
    ...
    getvmstats(pid, &st1);
    printf("  Sequential FCFS: disk_reads=%d disk_writes=%d avg_lat=%d.%d\n",
           st1.disk_reads, st1.disk_writes, st1.avg_disk_latency/100, st1.avg_disk_latency%100);

    if (st1.avg_disk_latency >= ROTATIONAL_C)
        printf("  PASS: avg_disk_latency >= C*100 (%d >= %d)\n",
               st1.avg_disk_latency, ROTATIONAL_C * 100);
    else
        printf("  FAIL: avg_disk_latency %d < minimum %d\n",
               st1.avg_disk_latency, ROTATIONAL_C );

    // ------------------------------------------------------------------
    printf("[2] Scattered access should yield higher avg_disk_latency than sequential\n");
    ...
    getvmstats(pid, &st2);
    printf("  Scattered SSTF: disk_reads=%d disk_writes=%d avg_lat=%d.%d\n",
           st2.disk_reads, st2.disk_writes, st2.avg_disk_latency/100, st2.avg_disk_latency%100);

    // avg_disk_latency after scatter >= after sequential (cumulative average)
    ...
    if (sstf_lat <= fcfs_lat)
        printf("  PASS: SSTF avg_disk_latency <= FCFS avg_disk_latency\n");
    else
...
test_pa4_20(void)
{
    ...
    for (int c = 0; c < NCHILDREN_20; c++) {
        ...
        printf("  child %d (%s): pid=%d disk_reads=%d disk_writes=%d"
        ...
        if (r->disk_reads  == 0)  { printf("  FAIL: child %d no disk_reads\n",  c); all_ok = 0; }
        if (r->disk_writes == 0)  { printf("  FAIL: child %d no disk_writes\n", c); all_ok = 0; }
    }
    ...
    int unique_disk_reads = 1;
    for (int a = 0; a < NCHILDREN_20 && unique_disk_reads; a++)
        for (int b = a+1; b < NCHILDREN_20; b++)
            if (reports[a].pid == reports[b].pid) unique_disk_reads = 0;
    if (unique_disk_reads)
...
#define FRAME_LIMIT_21 128
// 3x frame limit forces heavy eviction
#define NUM_PAGES_21   (50)
#define PASSES_21      4
#define RAID0_21       0
#define SSTF_21        1
...
test_pa4_21(void)
{
    ...
    printf("  Disk stats: disk_reads=%d disk_writes=%d latency=%d.%d\n",
    ...
    if (dk_after.disk_writes > dk_before.disk_writes)
        printf("  PASS: disk disk_writes increased (%d)\n",
               dk_after.disk_writes - dk_before.disk_writes);
    else
        printf("  FAIL: no new disk disk_writes\n");

    if (dk_after.disk_reads > dk_before.disk_reads)
        printf("  PASS: disk disk_reads increased (%d)\n",
               dk_after.disk_reads - dk_before.disk_reads);
    else
        printf("  FAIL: no new disk disk_reads\n");
    ...
    int child = fork();
    if (child == 0) {
        // Child overdisk_writes with different pattern
        for (int i = 0; i < NUM_PAGES_21; i++)
...
test_pa4_22(void)
{
    ...
    printf("  FCFS: disk_reads=%d disk_writes=%d avg_lat=%d.%d ticks\n",
    ...
    printf("  SSTF (cumulative): disk_reads=%d disk_writes=%d avg_lat=%d.%d ticks\n",
           st_sstf.disk_reads, st_sstf.disk_writes,
           st_sstf.avg_disk_latency/100, st_sstf.avg_disk_latency%100);

    // ------------------------------------------------------------------
    printf("[3] SSTF avg_disk_latency not worse than FCFS\n");
    ...
    // ------------------------------------------------------------------
    printf("[6] avg_disk_latency always >= rotational delay (700 in 100x units)\n");
    if (st_fcfs.avg_disk_latency >= 5)
        printf("  PASS: FCFS latency >= C*100\n");
    else
        printf("  FAIL: FCFS latency %d < 700\n", st_fcfs.avg_disk_latency);

    if (st_sstf.avg_disk_latency >= 5)

...
test_pa4_11(void)
{
    ...
    printf("\nDisk Stats (FCFS): disk_reads=%d, disk_writes=%d, Avg Latency=%d.%d ticks\n",
    ...
    getvmstats(getpid2(),&diskstat);
    printf("\nDisk Stats (SSTF): disk_reads=%d, disk_writes=%d, Avg Latency=%d.%d ticks\n",
           diskstat.disk_reads, diskstat.disk_writes, diskstat.avg_disk_latency / 100, diskstat.avg_disk_latency % 100);
...
test_pa4_12(void)
{
    ...
    printf("  FCFS stats -> disk_reads=%d disk_writes=%d avg_disk_latency=%d.%d ticks\n",
           st.disk_reads, st.disk_writes, st.avg_disk_latency / 100, st.avg_disk_latency % 100);

    if (st.disk_writes > 0)
        printf("  PASS: disk_disk_writes > 0 (swap-out happened)\n");
    else
        printf("  FAIL: disk_disk_writes == 0\n");

    if (st.disk_reads > 0)
        printf("  PASS: disk_disk_reads > 0 (swap-in happened)\n");
    else
        printf("  FAIL: disk_disk_reads == 0\n");

    if (st.avg_disk_latency > 0)
        printf("  PASS: avg_disk_latency > 0\n");
    else
        printf("  FAIL: avg_disk_latency == 0 — latency model not running\n");
...
test_pa4_13(void)
{
    ...
    printf("  FCFS  -> disk_reads=%d disk_writes=%d avg_disk_latency=%d.%d ticks\n",
    ...
    // avg_disk_latency is already an average, use the post value
    sstf_st.avg_disk_latency = after_sstf.avg_disk_latency;

    printf("  SSTF  -> disk_reads=%d disk_writes=%d avg_disk_latency=%d.%d ticks\n",
    ...
    if (after_sstf.avg_disk_latency <= fcfs_st.avg_disk_latency)
        ...
        printf("  NOTE: SSTF avg_disk_latency > FCFS avg_disk_latency — workload"
    ...
    if (fcfs_st.avg_disk_latency >= 0.5)
        printf("  PASS: FCFS avg_disk_latency includes rotational delay\n");
    else
        printf("  FAIL: FCFS avg_disk_latency %d.%d < 7.00 ticks\n",
               fcfs_st.avg_disk_latency/100, fcfs_st.avg_disk_latency%100);
